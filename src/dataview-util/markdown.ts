...
export class Link {
  ...
  public type: 'file' | 'header' | 'block' = 'file';

  public withListCache(id?: string, _itemText?: string): Link {
    return new Link({
      path: this.path,
      display: _itemText ?? this.display ?? this.path,
      subpath: id,
      embed: this.embed,
      type: 'block'
    });
  ...
  public static withLinkCache(cache: LinkCache): Link {
    return Link.file(cache.link, false, cache.displayText);
  }

  public withSectionCache(cache: SectionCache, text: string): Link {
    switch (cache.type) {
      case 'heading':
      ...
      case 'block':
        return new Link({
          path: this.path,
          display: this.display ?? this.path,
          subpath: cache.id,
          embed: this.embed,
          type: 'block'
        });
  ...
  public static file(path: string, embed = false, display?: string): Link {
    return new Link({
      path,
      embed,
      display: display ?? '',
...
export class ListItem {
  ...
  public file(): Link {
    return this.link?.toFile() ?? Link.fromObject({});
  }
