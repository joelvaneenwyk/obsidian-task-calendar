...
if (firstDayOfWeek) {
  if (firstDayOfWeek.match(/[|\\0-6]/g) == null) {
...
if (startPosition) {
  if (!startPosition.match(/\d{4}-\d{1,2}/g)) {
    dv.span(
...
function getMeta(tasks) {
  for (let i = 0; i < tasks.length; i++) {
    ...
    tasks[i].text = tasks[i].text.replaceAll(']]', '');
    tasks[i].text = tasks[i].text.replace(/\[.*?\]/g, '');
  }
}
...
function getTasks(date) {
    ...
    .filter((t) => t.completed && t.checked && t.completion && moment(t.completion.toString()).isSame(date))
    .sort((t) => t.completion);
  doneWithoutCompletionDate = tasks
    .filter((t) => t.completed && t.checked && !t.completion && t.due && moment(t.due.toString()).isSame(date))
    .sort((t) => t.due);
  done = done.concat(doneWithoutCompletionDate);
  due = tasks
    .filter((t) => !t.completed && !t.checked && !t.recurrence && t.due && moment(t.due.toString()).isSame(date))
    .sort((t) => t.due);
  recurrence = tasks
    .filter((t) => !t.completed && !t.checked && t.recurrence && t.due && moment(t.due.toString()).isSame(date))
    .sort((t) => t.due);
  overdue = tasks
    .filter((t) => !t.completed && !t.checked && t.due && moment(t.due.toString()).isBefore(date))
    .sort((t) => t.due);
  start = tasks
    .filter((t) => !t.completed && !t.checked && t.start && moment(t.start.toString()).isSame(date))
    .sort((t) => t.start);
  scheduled = tasks
    .filter((t) => !t.completed && !t.checked && t.scheduled && moment(t.scheduled.toString()).isSame(date))
    .sort((t) => t.scheduled);
  ...
  cancelled = tasks
    .filter((t) => !t.completed && t.checked && t.due && moment(t.due.toString()).isSame(date))
    .sort((t) => t.due);
  dailyNote = tasks
    .filter((t) => !t.completed && !t.checked && t.dailyNote && moment(t.dailyNote.toString()).isSame(date))
    .sort((t) => t.dailyNote);
}
...
function getMonth(tasks, month) {
  removeExistingView();
  var currentTitle =
    '<span>' + moment(month).format('MMMM') + '</span><span> ' + moment(month).format('YYYY') + '</span>';
...
function getWeek(tasks, week) {
  removeExistingView();
  var currentTitle =
    '<span>' + moment(week).format('YYYY') + '</span><span> ' + moment(week).format('[W]w') + '</span>';
  rootNode.querySelector('button.current').innerHTML = currentTitle;
  var gridContent = '';
...
function getList(tasks, month) {
  ...
  for (i = 0; i < moment(month).endOf('month').format('D'); i++) {
    ...
    if (moment().format('YYYY-MM-DD') == currentDate) {
      ...
      var overdueDetails =
        ...
        '</details>';
      var todayDetails =
        "<details open class='today'><summary>Today</summary>" + setTaskContentContainer(currentDate) + '</details>';
      ...
      var upcomingDetails =
        "<details open class='upcoming'><summary>Upcoming</summary>" + upcomingContent + '</details>';
  ...
  rootNode
    .querySelector('span')
    .appendChild(dv.el('div', listContent, { cls: 'list', attr: { 'data-month': monthName } }));
  setStatisticValues(
    dueCounter,
    doneCounter,
    overdueCounter,
