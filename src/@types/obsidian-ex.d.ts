...
type CommandsApp = 'app:open-vault' | 'app:reload';
type CommandsPluginTasks = 'obsidian-tasks-plugin:edit-task' | 'obsidian-tasks-plugin:toggle-done';
type KnownCommands = CommandsApp | CommandsPluginTasks;
...
interface Commands {
   ...
   */
  commands: PartialRecord<KnownCommands, Command>;
  /**
   ...
   */
  editorCommands: Record<KnownCommands, Command>;
  /**
...
type CommunityPlugin = 'obsidian-tasks-plugin';
type InternalPlugin =
  | 'audio-recorder'
  | 'backlink'
  | 'bookmarks'
...
type ObsidianPlugin = InternalPlugin | CommunityPlugin;

interface PluginEx extends Plugin {
  instance?: PluginEx;
  options?: Record<string, any>;
  openGlobalSearch?: (tag: string) => void;
}

interface DataViewPlugin extends PluginEx {}

type PartialRecord<K extends keyof any, T> = {
  [P in K]?: T;
};

interface CommunityPluginMapping extends PartialRecord<CommunityPlugin, PluginEx> {
  dataview?: PluginEx;
}

interface InternalPluginMapping extends PartialRecord<InternalPlugin, PluginEx> {
  backlink?: PluginEx;
}
...
interface InternalPlugins extends Events {
   ...
   * @param id - ID of the plugin to get
   */
  getPluginById: (id: InternalPlugin) => PluginEx;
...
declare class MetadataTypeManager extends Events {
  /**
   * Reference to App
   */
  app: App;
...
interface Plugins {
   ...
   */
  plugins: CommunityPluginMapping;
  /**
