...
export class TimelineSettings implements __Options {
  ...
  sort = (t1: TaskDataModel, t2: TaskDataModel) => {
  ...
  postProcessor() {
    ...
    const dailyNoteFolder =
      this.dailyNoteFolder === ''
        ? ''
        : this.dailyNoteFolder.endsWith('/')
        ? this.dailyNoteFolder
        : this.dailyNoteFolder + '/';
    if (this.dailyNoteFormat && this.dailyNoteFormat !== '') this.taskFiles.add(dailyNoteFolder + dailyNoteFileName);
  }
}
