...
export type TaskStatusType = 'due' | 'overdue' | 'scheduled' | 'start' | 'done' | 'unplanned' | 'process' | 'cancelled';
...
export const TaskStatusCollection: string[] = [
  TaskStatus.due,
...
export class TaskRegularExpressions {
  ...
  public static readonly listMarkerRegex = /([-*]|\d+\.)/;
  ...
  public static readonly blockLinkRegex = / \^[a-zA-Z\d-]+/u;
  ...
  public static readonly recurrenceRegex = /🔁 ?([a-z\d, !]+)/iu;

  // regex from @702573N
  public static readonly hexColorRegex = /([a-fA-F\d]{6}|[a-fA-F\d]{3})\/(.*)/;
  ...
  public static readonly outerLinkRegex =
    // #todo #jve Removed a '\6' which was invalid but not sure what the intent was.
    // eslint-disable-next-line regexp/no-super-linear-backtracking
    /\[((?:\[[^\]]*\]|[^[\]])*)\]\([ \t]*<?((?:\([^)]*\)|[^()\s])*?)>?[ \t]*((['"])(.*?)[ \t]*)?\)/g;
...
export interface SortableTask {
  /**
   * Sort order for the task.
   */
  order: number;
}

//const defaultSort = (t1: TaskDataModel, t2: TaskDataModel) => { return t1.order - t2.order; };
export interface TaskDataModel extends STask, SortableTask {
  /**
   * Whether this task is in a daily note.
   */
  dailyNote: boolean;
  /**
  ...
  /**
   * Recurrence represented as a string.
   */
  recurrence: string;
  /**
   * Frontmatter associated with the task.
   */
  frontMatter: Record<string, string>;

  /**
   * Whether or not this is a sub-task.
   */
  isTasksTask: boolean;

  /**
   * Status marker of the task used for display.
   */
  statusMarker: string;

  /**
   * Any dates associated with the task.
   */
