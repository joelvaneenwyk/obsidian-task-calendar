...
export class TimelineView extends React.Component<TimelineProps, TimelineStates> {
  ...
  render(): React.ReactNode {
    ...
    if (!involvedDates.has(moment().format(innerDateFormat))) involvedDates.add(moment().format(innerDateFormat));
    ...
    //const taskOfToday = taskList.filter(TaskMappable.filterDate(moment()));
    ...
    if (
      !this.props.userOptions.useFileBadge &&
      ...
      !this.props.userOptions.useTags
    )
      styles.push('noInfo');
    else {
    ...
    const counterFilter =
      this.state.filter.length === 0 ? '' : this.state.filter + this.props.userOptions.counterBehavior;
    const todayFocus = this.state.todayFocus ? 'todayFocus' : '';
    return (
      <div className={`taskido ${baseStyles} ${counterFilter} ${todayFocus}`} id={`taskido${new Date().getTime()}`}>
            ...
            <span>
              {years.map((y, i) => (
                <TaskListContext.Provider
                  value={{
                    taskList: taskList.filter(TaskMappable.filterYear(moment().year(y))),
                    entryOnDate:
                      this.props.userOptions.entryPosition === 'top'
                        ? firstDay!
                        : this.props.userOptions.entryPosition === 'bottom'
                        ? lastDay!
