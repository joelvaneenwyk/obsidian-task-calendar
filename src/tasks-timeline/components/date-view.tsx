...
import {
  QuickEntryHandlerContext,
  TaskListContext,
  TodayFocusEventHandlersContext,
  UserOptionContext
} from './context';
...
class QuickEntry extends React.Component<Record<string, unknown>, QuickEntryState> {
  ...
  onQuickEntryFileSelectChange() {
    if (!this.fileSelect.current) return;
    this.setState({
      selectedFile: this.fileSelect.current?.value
    });
      ...
      <div className="quickEntryPanel" ref={this.quickEntryPanel}>
        <div className="left">
          <div className="actionSelect">
            ...
            >
              <option value={'append'} key={1}>
                New Task
              </option>
              <option value={'filter'} key={2}>
                Filter
              </option>
            </select>
                ...
                <UserOptionContext.Consumer>
                  {({ taskFiles }) =>
                    [...taskFiles].map((f, i) => {
                      ...
                      const parentFolder = !f.split('/')[f.split('/').length - 2]
                        ? ''
          ...
          <div className="left">
            {this.state.action === 'append' ? (
              <input
                ...
                ref={this.textInput}
                onInput={this.onQuickEntryNewTaskInput}
                onKeyUp={this.onQuickEntryNewTaskKeyUp}
                onFocus={this.onQuickEntryPanelFocus}
                onBlur={this.onQuickEntryPanelBlur}
              />
            ) : (
              <div className="left">
                {this.state.filters.map((f, i) => {
                  if (f === 'date') {
                    return (
                      <div key={i} className="dateFilter">
                        <a key={0}>From: </a>
                        <input key={1} type="date" onChange={(e) => (this.dateFilter[0] = e.target.value)} />
                        <a key={2}> To: </a>
                        <input key={3} type="date" onChange={(e) => (this.dateFilter[1] = e.target.value)} />
                      </div>
                    );
                  } else if (f === 'priority') {
                    return (
                      <div key={i} className="actionSelect">
                        <a>Priorities: </a>
                        <MultiSelect
                          key={1}
                          name="priorityFilter"
                          className=""
                          options={['+', '1', '2', '3', '4']}
                          visual={['➕', 'High', 'Medium', 'None', 'Low']}
                          fallbackValueIndex={0}
                          selectChangeHandler={(res) => {
                            this.priorityFilter = res;
                          }}
                        />
                      </div>
                    );
    ...
    return (
      <div className="left MultiSelect">
        <select
          ...
          ref={this.selectElem}
          onClick={() => {
            if (this.selectElem.current !== null) this.selectElem.current.selectedIndex = this.props.fallbackValueIndex;
          }}
          onChange={(e) => {
        ...
        {this.state.results.map((f) => (
          <a>{this.props.visual[this.props.options.indexOf(f)] + '🎯'}</a>
        ))}
      </div>
    );
  }
...
class CounterItem extends React.Component<CounterProps> {
  render(): React.ReactNode {
    return (
      <div className="counter" id={this.props.id} aria-label={this.props.ariaLabel} onClick={this.props.onClick}>
