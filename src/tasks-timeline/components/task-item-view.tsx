...
import { TasksTimelineView } from 'views';
...
export class TaskItemView extends React.Component<TaskItemProps, TaskItemState> {
  ...
  render(): React.ReactNode {
    ...
    const color = item.frontMatter['color'];
          ...
          return (
            <UserOptionContext.Consumer>
              {({ dateFormat, hideTags, useBuiltinStyle }) => (
                <div
                  data-line={line}
                  data-task={item.statusMarker}
                  data-col={col}
                  data-link={link}
                  data-dailynote={isDailyNote}
                  className={`task ${item.status}`}
                  style={{ '--task-color': color || 'var(--text-muted)' } as React.CSSProperties}
                  aria-label={ariaLabel}
                >
                  <StripWithIcon
                    onToggle={onToggleTask}
                    useBuiltinStyle={useBuiltinStyle}
                    marker={item.statusMarker}
                    status={item.status}
                  />
                  <div className="lines" onClick={openTaskFile}>
                    ...
                    <div className="line info">
                      {callbacks.handleModifyTask && <ModifyBadge onClick={onModifyTask}></ModifyBadge>}
                      {item.created && (
                        <DateStatusBadge //onClick={openTaskFile}
                          className="relative"
                          ariaLabel={'create at ' + item.created.format(dateFormat)}
                          label={getRelative(item.created)}
                          icon={Icons.taskIcon}
                        />
                      )}
                      {item.start && (
                        <DateStatusBadge //onClick={openTaskFile}
                          className="relative"
                          ariaLabel={'start at ' + item.start.format(dateFormat)}
                          label={getRelative(item.start)}
                      ...
                      {item.scheduled && (
                        <DateStatusBadge //onClick={openTaskFile}
                          className="relative"
                          ariaLabel={'scheduled to ' + item.scheduled.format(dateFormat)}
                          label={getRelative(item.scheduled)}
                      ...
                      {item.due && (
                        <DateStatusBadge //onClick={openTaskFile}
                          className="relative"
                          ariaLabel={'due at ' + item.due.format(dateFormat)}
                          label={getRelative(item.due)}
                          icon={Icons.dueIcon}
                        />
                      )}
                      {item.completion && (
                        <DateStatusBadge //onClick={openTaskFile}
                          className="relative"
                          ariaLabel={'complete at ' + item.completion.format(dateFormat)}
                          label={getRelative(item.completion)}
                          icon={Icons.doneIcon}
                        />
                      )}
                      ...
                      {item.recurrence && (
                        <DateStatusBadge //onClick={openTaskFile}
                          className="repeat"
                          ariaLabel={'recurrent: ' + item.recurrence.replace(recurrenceSymbol, '')}
                      ...
                      {item.priority && (
                        <DateStatusBadge //onClick={openTaskFile}
                          className="priority"
                          ariaLabel={'priority: ' + item.priority}
                      ...
                      {[...new Set(tags)]
                        ...
                        .map((t, i) => {
                          return <TagBadge tag={String(t)} key={i} />;
                        })}
                    </div>
                  </div>
                </div>
...
class Content extends React.Component<ContentProps> {
  render(): React.ReactNode {
    ...
    MarkdownRenderer.renderMarkdown(this.props.display, cont, this.props.fileName, TasksTimelineView.getActiveView()!);
    ...
    return (
      <div className="timeline">
        <input
          id="statusMarker"
          type="checkbox"
          className={this.props.useBuiltinStyle ? 'icon' : ''}
          data-task={this.props.marker}
          defaultChecked={this.props.marker !== ' '}
          onClick={() => {
        ...
        {this.props.useBuiltinStyle && (
          <label
            htmlFor="statusMarker"
            className="icon"
            onClick={() => {
              if (this.props.useBuiltinStyle) this.props.onToggle();
            }}
          >
            {Icons.getTaskStatusIcon(this.props.status)}
          </label>
        )}
      </div>
    );
    ...
    return (
      <UserOptionContext.Consumer>
        {({ tagPalette }) => {
          ...
          let tagText: string = '';
          try {
            tag = String(this.props.tag);
            tagText = tag.replace('#', '');
          } catch (tagConversionError) {
            console.debug(
              `Invalid tag '${this.props.tag}' in task item view for '${
                this.context
              }': ${tagConversionError?.toString()}`
            );
          }
...
class ModifyBadge extends React.Component<ModifyBadgeProps> {
  render(): React.ReactNode {
    return (
      <a aria-label="Modify Task" onClick={this.props.onClick}>
        ✏️
      </a>
    );
  }
